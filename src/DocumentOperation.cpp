...
int DocumentOperation::SearchLeak(const std::string& str_DocPath)
{
    ...
    if(str_SimilarDoc=="")
    {
        ...
        for(int i=0; i<vec_SimilarDocument.size(); i++)
        {
            ...
            std::wcout<<L"similarity between "<<StringUtil::ConvertCharArraytoWString(pch_DocPath)<<L" and "<<StringUtil::ConvertCharArraytoWString(pch_SimDocPath)<<L" is "<<100*similarDoc.f_similarity<<"%"<<std::endl;
            ...
            for(int j=0; j<n_SameSize; j++)
            {
                ...
                std::wstring wstr_SameTextSearch = StringUtil::ConvertCharArraytoWString(pch_OriginWord);
                ...
                std::wstring wstr_SameTextDB = StringUtil::ConvertCharArraytoWString(pch_DBWord);
    ...
    else
    {
        ...
        std::wcout<<L"xxxxxx "<<StringUtil::ConvertCharArraytoWString(pch_DocName) <<L" is similar to "<<StringUtil::ConvertCharArraytoWString(pch_SimilarDocName)<<std::endl;
    }
    return 0;
}
